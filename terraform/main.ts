...
class AttestationManagerStack extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    const instanceRole = new IamRole(this, 'attestation-manager-role', {
      name: 'attestation-manager-role',
      assumeRolePolicy: JSON.stringify({
        Version: '2012-10-17',
        Statement: [{
          Action: 'sts:AssumeRole',
    ...
    new IamRolePolicy(this, 'attestation-manager-policy', {
      name: 'attestation-manager-policy',
    ...
    const instanceProfile = new IamInstanceProfile(this, 'attestation-manager-profile', {
      name: 'attestation-manager-profile',
...
if [ ! -f /etc/attestation-manager/operator.key ]; then
  openssl ecparam -name prime256v1 -genkey -noout -out /etc/attestation-manager/operator.key
